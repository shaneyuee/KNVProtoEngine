...
namespace knv
{
	...
	{
		static map<uint64_t, string> g_knv_cmd_maps;
		return g_knv_cmd_maps;
	}

	map<uint64_t, string> &GetProtoErrorMap()
	{
		static map<uint64_t, string> g_knv_errcode_maps;
		return g_knv_errcode_maps;
	}

	const char *GetCmdName(uint64_t c)
	{
		map<uint64_t, string> &g_knv_cmd_maps = GetProtoCommandMap();
	...
	{
		map<uint64_t, string> &g_knv_errcode_maps = GetProtoErrorMap();
